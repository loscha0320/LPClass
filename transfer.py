...
# x_train = x_train[:,0:int(x_train.shape[1]*0.8),:]
...
# print('test')
# trainer.test(brickSize, batchSize, x_test, y_test)

## transfer
print('transfer')
## load the model
dict = torch.load('model_srnn_MIT.pt')
# dict = torch.load('model_srnn_Epilepsy.pt')
dict["W"] = nn.Parameter(torch.randn([hiddenDim1, numClasses]))
dict["B"] = nn.Parameter(torch.randn([numClasses]))

srnn2.load_state_dict(dict)
srnn2.eval()

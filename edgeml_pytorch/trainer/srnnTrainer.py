        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % valStep == 0 or (epoch == epochs - 1):
                ...
                for i in range(len(x_val_batches)):
                    ...
                    numCorrect += count
                    prob_all.extend(predictions)
                    label_all.extend(target)
                print("Validation accuracy: %f" % (numCorrect / x_val.shape[1]))
                print("Validation F1-Score:{:.4f}".format(f1_score(label_all, prob_all, average="weighted")))
                print("Confusion Matrix:")
                print(confusion_matrix(label_all, prob_all))


    def test(self, brickSize, batchSize, x_val, y_val):
        L = self.srnnObj.outputDim
        assert batchSize >= 1, 'Batch size should be positive integer'
        assert x_val.ndim == 3, 'Expected validation data to be of rank 3'
        assert y_val.ndim == 2, 'Expected validation labels to be of rank 2'
        ...
        for i in range(len(x_val_batches)):
            ...
            prob_all.extend(predictions)
            label_all.extend(target)
        print("Test accuracy: %f" % (numCorrect / x_val.shape[1]))
        print("Test F1-Score:{:.4f}".format(f1_score(label_all, prob_all, average="weighted")))
        print("Confusion Matrix:")
        print(confusion_matrix(label_all, prob_all))
